...
A candidate must have a concrete, discrete advocacy signal to qualify for this report at all: a written recommendation, an accepted skill endorsement, or an invite note. High message volume and general responsiveness are NOT qualifying signals on their own — someone {{OWNER_NAME}} talks to constantly is already an active relationship, not a hidden one. Use message data only as supporting context (recency, a snippet) for someone who already qualifies through one of the three real signals above, never as the reason they're included. If a genuinely-qualifying candidate is also among {{OWNER_NAME}}'s most frequent message contacts — clearly part of their regular, already-active inner circle — deprioritize them below other qualifying candidates; include them only if no one else in the data qualifies. Check the most recent message DATE for each candidate — if there's no contact in the last 12 months, label them dormant and recommend reconnection before any ask, never a direct ask. Never quote recommendation text directly — summarize what they said in your own words to avoid misattribution. Never list {{OWNER_NAME}} as their own advocate — exclude any entry matching that name. Never name a specific third party mentioned in message content as an introduction target — you can't confirm they aren't already {{OWNER_NAME}}'s own connection; keep introduction asks general.
...
Be specific. Use names. Every line should be immediately usable. Ground every "What you can offer them" in a real, specific detail from that person's own data — never a generic category-shaped offer ("introductions back," "a co-created conversation," "a credible voice on X") that could apply to any two founders in similar roles.`,
        ...
        <div className="no-print" style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(2,8,18,0.97)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 24 }}>
          <div style={{ background: `linear-gradient(160deg, #0f2040 0%, #0a1628 100%)`, border: `1px solid ${BLUE_BRIGHT}66`, borderRadius: 20, padding: "40px 44px", maxWidth: 440, width: "100%", boxShadow: `0 0 80px rgba(65,161,232,0.15), 0 24px 60px rgba(0,0,0,0.8)`, animation: "fadeIn 0.2s ease-out", textAlign: "center" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              ...
              <button onClick={() => { setShowExitModal(false); setPdfSaveClicked(false); }} style={{ width: "100%", padding: "14px 22px", background: "transparent", border: `1px solid ${BORDER}`, borderRadius: 10, color: MUTED, fontSize: 14.5, fontWeight: 700, cursor: "pointer" }}>Back to my report →</button>
            </div>
          </div>
        </div>
      )}
